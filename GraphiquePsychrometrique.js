...
function tocm(px) {
    return px / 118.1144781144781;
}

function topx(cm) {
    return cm * 118.1144781144781;
}
...
function GraphPsychrometrique(Psychrometrique) {
    ...
    this.Origin = new point(3, 2);

// Taille de l'image en cm soit une feuille A4

    this.Largeur = 21;

    this.Longueur =  this.Largeur * Math.sqrt(2);
    ...
    var XtoGraph = function (X) {
        return topx(RapportX * (X - this.Xmin + this.Origin.x) );
    }

    /* Calcul de convertion de l'axe Y vers le dession pour l'appel des fonctions Draw ou Write*/
    var YtoGraph = function (Y) {
        return this.canvas.height - topx(RapportY * (Y - this.Ymin + this.Origin.y) );
    }


    this.GraphtoX = function (X) {
        var _Rapport = this.canvas.clientWidth / (this.Xmax - this.Xmin);
        X = ((X ) / _Rapport ) * ReductionImage - this.Origin.x + this.Xmin;
        if (X < this.Xmin)
            return this.Xmin;
    ...
    this.GraphtoY = function (Y) {
        ...
        Y = ((this.canvas.clientHeight - Y ) / _Rapport ) * ReductionImage + this.Ymin - this.Origin.y;
        if (Y < this.Ymin)
            return this.Ymin;
    ...
    var drawAxeT = function () {
        ...
        for (var T = this.Xmin; T <= this.Xmax; T += step) {
            ...
            if (T / 5 == Math.round(T / 5)) {
                ...
                this.writeTextGraph(toString(T, 3), new point(X.x-1.1, X.y-1),40,0,'#2dff00');// Write legende
                this.writeTextGraph(toString(T, 3), new point(Y.x-1.1, Y.y+0.2),40,0,'#2dff00');// Write legende
    ...
    var drawAxeR = function () {
        ...
        for (var R = this.Ymin; R <= this.Ymax; R += step) {
            ...
            var Pvs = Psychrometrique.calcPvFromRHR( R / 1000,100);
            var Tsec = Psychrometrique.calcTfromPvs(Pvs);
            if (Tsec < this.Xmin)
                Tsec = this.Xmin;
            if (Tsec > this.Xmax)
                Tsec = this.Xmax;
            ...
            if (R / 5 == Math.round(R / 5)) {
                ...
                this.writeTextGraph(String(R), new point(Y.x+0.5, Y.y-0.3),40,0,'#0000ff');
            }
    ...
    this.drawLineGraph = function (X, Y, color, width) {
        ...
        if (width === undefined)
            width = 2;
        this.context.beginPath();
        var x = XtoGraph.call(this, X.x);
        var y = YtoGraph.call(this, X.y);
        this.context.moveTo(x, y);
        x = XtoGraph.call(this, Y.x);
        y = YtoGraph.call(this, Y.y);
    ...
    this.writeTextGraph = function (Text, Point, Width, Rotate, Color) {
        if (Width === undefined)
            Width = 50;
        if (Rotate === undefined)
            Rotate = 0;
      ...
      //  this.context.moveTo(XtoGraph.call(this, Point.x), YtoGraph.call(this, Point.y));
